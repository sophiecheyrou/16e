const CONFIG={API_URL:'https://script.google.com/macros/s/AKfycbylb1Okn8UXSz3cnDPyPY7a6eQ1N-p2lDwj67hjtE5n5Z-JkbqesL6ERrWWFX_bSdSf/exec',ADMIN_PIN:'1600'};
...
async function getData(){
  if(CONFIG.API_URL){
    ...
    const j=await r.json(); if(j.ok===false)throw new Error(j.error||'Accès refusé'); return j;
  }
...
function render(d){
  ...
  $('#proofs').innerHTML=d.proofs.slice().reverse().map(p=>`<article class="proof-card"><div>${p.photo?`<a class="secondary link-button" href="${p.photo}" target="_blank" rel="noopener">Ouvrir la photo</a>`:'<div class="panel-icon">📷</div>'}</div><div class="proof-meta"><b>${p.team} — ${p.place}</b><small>Code : ${p.code||'—'}</small><small>${new Date(p.time).toLocaleString('fr-FR')}</small>${statusLabel(p.status)}<div class="proof-actions"><button class="approve" data-status="valid" data-id="${p.id}">✓ Valider</button><button class="reject" data-status="rejected" data-id="${p.id}">✕ Refuser</button><button class="secondary" data-status="pending" data-id="${p.id}">↺ Remettre en attente</button></div></div></article>`).join('');
  document.querySelectorAll('[data-status]').forEach(b=>b.onclick=()=>setStatus(b.dataset.id,b.dataset.status));
}
async function setStatus(id,status){
  try{
    const body=new URLSearchParams({action:'updatestatus',pin:CONFIG.ADMIN_PIN,id,status});
    const r=await fetch(CONFIG.API_URL,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:body.toString()});
    const j=await r.json();if(!r.ok||j.ok===false)throw new Error(j.error||'Mise à jour impossible');
    toast(status==='valid'?'Preuve validée':status==='rejected'?'Preuve refusée':'Preuve remise en attente');await refresh();
  }catch(e){toast(e.message||'Mise à jour impossible')}
}
